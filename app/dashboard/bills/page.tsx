        ...
        <div className="min-h-screen bg-[#1a0e07] text-[#f5ede2] flex font-sans overflow-hidden">
            ...
            <main className="flex-1 flex flex-col min-w-0 relative">
                <DashboardHeader title="All Bills Management (Admin)" onAddBill={() => setShowAddBill(true)}>
                    <button onClick={() => refetch()} className="p-2 rounded-xl border border-border bg-[#1a0e07] hover:border-primary/40 text-muted-foreground hover:text-primary transition-all cursor-pointer" title="Refresh bills">
                ...
                <div className="flex-1 overflow-y-auto p-8 flex flex-col gap-6">
                    ...
                    <div className="bg-[#251508] border border-border rounded-2xl p-4 shadow-xl flex items-center justify-between gap-4 shrink-0">
                        <p className="text-xs text-muted-foreground font-mono">Admin view of all user & group bills across the system</p>
                    ...
                    {meta && (
                        <div className="bg-[#251508] border border-border rounded-2xl px-6 py-3.5 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-muted-foreground font-mono shadow-xl">
                            <div>
                                Showing <span className="font-bold text-foreground">{bills.length}</span> of <span className="font-bold text-foreground">{meta.total}</span> bills (Page <span className="font-bold text-primary">{meta.page}</span> of {meta.totalPages || 1})
                            </div>
