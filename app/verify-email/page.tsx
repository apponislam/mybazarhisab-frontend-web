...
function VerifyEmailContent() {
    ...
    const { data, error } = useVerifyEmailQuery(
        { email, token },
        { skip: !email || !token }
    );
    ...
    useEffect(() => {
        ...
        if (data?.success) {
            ...
            const errMsg = (error as any)?.data?.message || "Email verification failed.";
            toast.error(errMsg);
            router.replace("/login");
        }
    ...
    return (
        <div className="min-h-screen bg-[#120a05] text-slate-100 flex flex-col items-center justify-center p-4">
            <div className="flex flex-col items-center gap-3">
                ...
                <p className="text-sm font-medium text-slate-300 font-mono">Verifying your email address...</p>
            </div>
        </div>
    );
}
