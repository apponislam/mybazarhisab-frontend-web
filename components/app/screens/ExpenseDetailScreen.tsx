...
export function ExpenseDetailScreen({ entry, onBack, onEdit, onDelete }: { entry: MockBazarEntry; onBack: () => void; onEdit: () => void; onDelete: () => void }) {
        ...
        <ScreenShell scrollable>
            <div className="flex flex-col px-6 pt-12 pb-8">
                ...
                <div className="flex flex-col gap-3">
                    ...
                    <motion.div initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.28 }} className="flex items-center gap-3 p-4 rounded-xl border border-border bg-card" style={{ boxShadow: "0 2px 8px rgba(0,0,0,0.2)" }}>
                    ...
                    {entry.notes && (
                        <motion.div initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.34 }} className="flex items-start gap-3 p-4 rounded-xl border border-border bg-card" style={{ boxShadow: "0 2px 8px rgba(0,0,0,0.2)" }}>
                    ...
                    {confirmDelete && <DeleteConfirm label="Expense" onConfirm={onDelete} onCancel={() => setConfirmDelete(false)} />}
                </div>
            </div>
        </ScreenShell>
    );
