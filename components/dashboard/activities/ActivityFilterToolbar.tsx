...
export function ActivityFilterToolbar({
    ...
    const hasActiveFilters = Boolean(
        filters.type || filters.action || filters.userId || filters.groupId || filters.startDate || filters.endDate
    );
                            ...
                            <button
                                ...
                                className={`px-3.5 py-1.5 rounded-xl text-xs font-semibold whitespace-nowrap transition-all cursor-pointer border ${
                                    active
                                        ...
                                        : "bg-[#1a0e07] text-muted-foreground border-border hover:border-primary/40 hover:text-foreground"
                                }`}
                            >
                                {cat.label}
                            </button>
                ...
                <div className="flex items-center gap-2 shrink-0">
                    {hasActiveFilters && (
                        <button
                            ...
                            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-border bg-[#1a0e07] text-xs font-semibold text-muted-foreground hover:text-foreground hover:bg-white/5 transition-colors cursor-pointer"
                        >
                    ...
                    <button
                        ...
                        className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-destructive/15 border border-destructive/30 text-destructive text-xs font-bold hover:bg-destructive/25 transition-all cursor-pointer"
                    >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 pt-2 border-t border-border/60">
                ...
                <div className="relative flex items-center">
                    ...
                    <input
                        ...
                        className="w-full pl-8 pr-3 py-2 bg-[#1a0e07] border border-border rounded-xl text-xs outline-none text-foreground font-mono"
                    />
                </div>
                ...
                <div className="flex items-center gap-2 bg-[#1a0e07] border border-border rounded-xl px-3 py-1.5">
                    ...
                    <select
                        ...
                        className="bg-transparent text-xs text-foreground outline-none cursor-pointer w-full font-mono"
                    >
