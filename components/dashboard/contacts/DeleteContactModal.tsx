...
import { TContact, useDeleteMessageMutation } from "@/redux/features/contact/contactApi";
...
export function DeleteContactModal({ contact, onClose }: DeleteContactModalProps) {
    const [deleteMessage, { isLoading }] = useDeleteMessageMutation();

    const handleDelete = async () => {
        if (!contact) return;
        try {
            await deleteMessage(contact._id).unwrap();
            toast.success("Message deleted successfully!");
            onClose();
        } catch (err: any) {
            toast.error(err?.data?.message || err?.message || "Failed to delete message");
        }
    };
                    ...
                    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-[#251508] border border-border rounded-3xl p-6 w-full max-w-md shadow-2xl flex flex-col gap-6">
                        <div className="flex items-center justify-between border-b border-border pb-4">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h3 className="text-lg font-bold text-foreground font-sans">Delete Message</h3>
                        ...
                        <div className="text-sm text-muted-foreground space-y-2 font-sans">
                            <p>
                                Are you sure you want to delete the support message from <strong className="text-foreground">{contact.name}</strong> ({contact.email})?
                            </p>
