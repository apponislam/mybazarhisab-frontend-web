    ...
    return (
        <div className="relative flex items-end bg-card border-t border-border px-2 pb-2 pt-1 flex-shrink-0" style={{ boxShadow: "0 -4px 24px rgba(0,0,0,0.4)" }}>
            ...
            <div className="flex-1 flex flex-col items-center pb-1">
                <motion.button onClick={onAdd} whileTap={{ scale: 0.9 }} className="w-14 h-14 rounded-full bg-primary flex items-center justify-center -mt-6 shadow-xl cursor-pointer" style={{ boxShadow: "0 4px 20px rgba(232,160,32,0.5)" }}>
                    <Plus className="w-7 h-7 text-primary-foreground" strokeWidth={2.5} />
                </motion.button>
                <span className="text-[10px] font-medium mt-1" style={{ fontFamily: "'DM Sans', sans-serif", color: "#a08060" }}>
                    Add
                </span>
            </div>
            {tabs.slice(2).map((t) => (
        ...
        <ScreenShell>
            <div className="flex flex-col flex-1 min-h-0 relative">
                ...
                <div className="flex-1 min-h-0 relative flex flex-col">
                    ...
                    {subScreen === "add-picker" && <AddPicker onExpense={() => setSubScreen("add-expense")} onBill={() => setSubScreen("add-bill")} onClose={() => setSubScreen(null)} />}
                </div>
                {showNav && (
                    <BottomNav
                        tab={tab}
