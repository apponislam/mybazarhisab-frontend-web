  ...
  return (
    <motion.button type="button" onClick={onClick} initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }}
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-muted-foreground mt-0.5 font-mono">{entry.quantity} {entry.unit} · {fmtDate(entry.date)}</p>
      </div>
      <div className="flex flex-col items-end gap-1 flex-shrink-0">
        <p className="text-sm font-bold text-primary font-mono">{fmtFull(total)}</p>
        <p className="text-xs text-muted-foreground font-mono">৳{entry.price}/{entry.unit}</p>
