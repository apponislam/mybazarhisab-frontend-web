...
import {
  ChevronUp, ChevronDown, ShoppingBag, Star, Calendar, TrendingUp,
  Minus, BookOpen, Package, Receipt, BarChart2
} from "lucide-react";
...
function Delta({ current, prev }: { current: number; prev: number }) {
  ...
  const up = pct >= 0;
  return (
    <span className="flex items-center gap-0.5 text-xs font-medium font-mono"
...
export function HomeTab({ stats }: { stats: GroupStats }) {
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0 overflow-y-auto">
      ...
      <div className="mx-6 h-px bg-border mb-1 flex-shrink-0" />
      <div className="flex flex-col px-6 pb-6 gap-4">
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}
          ...
          <p className="text-xs text-muted-foreground mb-1 uppercase tracking-widest font-mono">{mn} {yr} — Total</p>
