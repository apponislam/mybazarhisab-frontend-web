...
import {
  User, Shield, Info, Bell, BellOff, Mail, Globe, MapPin,
  FileText, LogOut, Trash2, AlertTriangle
} from "lucide-react";
...
export function ProfileTab({ onEditProfile, onChangePassword }: { onEditProfile: () => void; onChangePassword: () => void }) {
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0 overflow-y-auto">
      ...
      <div className="mx-6 h-px bg-border mb-4 flex-shrink-0" />
      ...
      <div className="flex flex-col px-6 pb-8 gap-5">
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.18 }}
          ...
          <div className="px-4 py-3 border-b" style={{ borderColor: "rgba(212,24,61,0.2)" }}>
            <p className="text-xs font-semibold uppercase tracking-widest font-mono" style={{ color: "#d4183d" }}>Danger Zone</p>
          </div>
          ...
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="flex gap-3">
              <button type="button" onClick={() => setShowDeleteConfirm(false)}
                className="flex-1 py-2.5 rounded-xl border border-border text-foreground text-sm font-medium hover:bg-secondary transition-all cursor-pointer"
                style={{ fontFamily: "'DM Sans', sans-serif" }}>Cancel</button>
              <button type="button"
                className="flex-1 py-2.5 rounded-xl bg-destructive text-white text-sm font-semibold hover:opacity-90 transition-all cursor-pointer"
