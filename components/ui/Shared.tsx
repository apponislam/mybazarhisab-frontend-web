...
export function PrimaryButton({ loading, label, loadingLabel, type = "submit", onClick, disabled }: {
...
export function BackButton({ onBack, label = "Back" }: { onBack: () => void; label?: string }) {
  return (
    <button type="button" onClick={onBack}
      className="flex items-center gap-1.5 text-muted-foreground hover:text-foreground transition-colors text-sm mb-5 cursor-pointer"
...
export function Toggle({ on, onToggle }: { on: boolean; onToggle: () => void }) {
  return (
    <button type="button" onClick={onToggle}
      className="w-12 h-6 rounded-full transition-all duration-300 flex items-center relative flex-shrink-0 cursor-pointer"
...
export function SettingsRow({ icon, label, sub, right, onClick, danger }: {
  icon: React.ReactNode; label: string; sub?: string; right?: React.ReactNode; onClick?: () => void; danger?: boolean;
}) {
  const Tag = onClick ? "button" : "div";
  return (
    <Tag type={onClick ? "button" : undefined} onClick={onClick}
      className={`w-full flex items-center gap-3 p-4 text-left transition-all ${onClick ? "hover:bg-primary/5 active:bg-primary/10 cursor-pointer" : ""}`}>
    ...
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
      ...
      <div className="flex gap-3">
        <button type="button" onClick={onCancel}
          className="flex-1 py-2.5 rounded-xl border border-border text-foreground text-sm font-medium transition-all hover:bg-secondary cursor-pointer"
          style={{ fontFamily: "'DM Sans', sans-serif" }}>Cancel</button>
        <button type="button" onClick={onConfirm}
          className="flex-1 py-2.5 rounded-xl bg-destructive text-white text-sm font-semibold transition-all hover:opacity-90 cursor-pointer"
