...
import {
  Home, ShoppingBag, Receipt, User, Plus, Search, X, LogOut,
  Settings, Lock, Mail, Phone, Camera
} from "lucide-react";
        ...
        <div className="max-w-6xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <span className="ml-3 hidden sm:inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full border border-primary/20 bg-primary/5 text-primary text-[10px] font-semibold font-mono">
                {stats.groupName} ⭐️
              </span>
            </div>
          </div>
            ...
            {[
              ...
              return (
                <button
                  ...
                  className="relative px-5 py-2.5 rounded-xl text-sm font-semibold transition-all flex items-center gap-2 cursor-pointer"
                  style={{ color: active ? "#e8a020" : "#a08060" }}
                >
                  {item.icon}
                  {item.label}
                  {active && (
                    <motion.div layoutId="web-nav-underline" className="absolute bottom-0 left-5 right-5 h-0.5 bg-primary rounded-full" />
                  )}
                </button>
              );
            })}
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:flex items-center gap-2">
              <button
                ...
                className="flex items-center gap-1 px-3 py-1.5 bg-primary text-primary-foreground text-xs font-bold rounded-lg transition-all hover:bg-accent cursor-pointer shadow-md shadow-primary/10"
              >
                <Plus className="w-3.5 h-3.5" /> Expense
              </button>
              <button
                onClick={() => setShowAddBill(true)}
                className="flex items-center gap-1 px-3 py-1.5 border border-accent text-accent text-xs font-bold rounded-lg transition-all hover:bg-accent/10 cursor-pointer"
              >
        ...
        <div className="md:hidden flex items-center justify-around bg-[#251508] border border-border p-1.5 rounded-2xl mb-6 select-none shadow-lg">
          {[
            ...
            <button
              key={item.id}
              onClick={() => setTab(item.id as never)}
              className="flex-1 py-2 flex flex-col items-center gap-1 rounded-xl text-xs font-medium cursor-pointer"
              style={{ color: tab === item.id ? "#e8a020" : "#a08060" }}
            >
              {item.icon}
          ...
          >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {calculations.settlements.map((s, idx) => (
                            <div key={idx} className="p-4 rounded-2xl border border-dashed border-primary/20 bg-primary/5 flex flex-col gap-2">
                              ...
                              <div className="text-xl font-bold text-primary font-mono text-center">
                                {fmtFull(s.amount)}
                              </div>
                            </div>
                          ))}
                        ...
                        <div key={e.id} className="p-3.5 rounded-xl bg-[#1a0e07] border border-[rgba(232,160,32,0.06)] flex items-center justify-between gap-3">
                          <div className="flex items-center gap-2.5 min-w-0">
                            ...
                            <div className="min-w-0">
                              ...
                              <p className="text-[10px] text-muted-foreground font-mono">{e.user.name} • {fmtDate(e.date)}</p>
                            </div>
                          </div>
            ...
            {tab === "expenses" && (
              <div className="flex-1 flex flex-col gap-6 min-h-0 bg-[#251508] border border-border rounded-3xl p-6 shadow-xl">
                {/* Search & Actions */}
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4 flex-shrink-0 select-none">
                  <div className="relative w-full sm:w-80">
                    ...
                    <input
                      ...
                      className="w-full pl-10 pr-4 py-2.5 bg-[#2e1a0a] border border-border rounded-xl text-sm placeholder-muted-foreground outline-none text-[#f5ede2]"
                    />
                  </div>
                      ...
                      {entries
                        ...
                        .map(e => (
                          <tr key={e.id} className="hover:bg-primary/5 transition-colors">
                            ...
                            <td className="p-4">
                              <div className="flex items-center gap-2">
                                <div className="w-6 h-6 rounded-full flex items-center justify-center font-bold text-[9px] text-[#f5ede2] flex-shrink-0" style={{ background: avatarColor(e.user.id) }}>
                          ...
                          <div key={b.id} className="rounded-2xl border border-border bg-[#1a0e07] p-5 flex flex-col justify-between gap-4 relative overflow-hidden">
                            <div>
                              <div className="flex items-center justify-between gap-2 mb-3">
                                <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-lg text-[10px] font-bold font-mono border"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
                ...
                <div className="bg-[#251508] border border-border rounded-3xl p-8 shadow-xl flex flex-col gap-6">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-lg font-bold text-foreground" style={{ fontFamily: "'DM Sans', sans-serif" }}>Edit Profile Details</h3>
                      <p className="text-xs text-muted-foreground" style={{ fontFamily: "'DM Sans', sans-serif" }}>Update your contact information</p>
                    </div>
                  </div>

                  <div className="flex items-center justify-center my-2">
                    <div className="relative cursor-pointer group">
                      <div className="w-20 h-20 rounded-full border-2 border-dashed border-primary/30 flex items-center justify-center font-bold text-2xl bg-[#1a0e07]">
                        AH
                      </div>
                      <div className="absolute -bottom-1 -right-1 w-6 h-6 rounded-full bg-primary flex items-center justify-center shadow-md"><Camera className="w-3.5 h-3.5 text-primary-foreground" /></div>
                    </div>
                  </div>

                  <form onSubmit={e => { e.preventDefault(); setProfileLoading(true); setTimeout(() => setProfileLoading(false), 1200); }} className="flex flex-col gap-4">
                    <FieldBox label="Full Name" focused={false}>
                      <div className="flex items-center">
                        <span className="pl-4 text-muted-foreground"><User className="w-4 h-4" /></span>
                        <input type="text" value={name} onChange={e => setName(e.target.value)} required className="flex-1 px-3 py-3.5 bg-transparent text-foreground placeholder-muted-foreground text-sm outline-none" />
                      </div>
                    </FieldBox>
                    ...
                    <FieldBox label="Email Address" focused={false}>
                      <div className="flex items-center">
                        ...
                        <input type="email" value={email} onChange={e => setEmail(e.target.value)} required className="flex-1 px-3 py-3.5 bg-transparent text-foreground placeholder-muted-foreground text-sm outline-none" />
                      </div>
                    </FieldBox>

                    <FieldBox label="Phone Number" focused={false}>
                      <div className="flex items-center">
                        <span className="pl-4 text-muted-foreground"><Phone className="w-4 h-4" /></span>
                        <input type="text" value={phone} onChange={e => setPhone(e.target.value)} required className="flex-1 px-3 py-3.5 bg-transparent text-foreground placeholder-muted-foreground text-sm outline-none" />
                      </div>
                    </FieldBox>
                ...
                <div className="bg-[#251508] border border-border rounded-3xl p-8 shadow-xl flex flex-col gap-6">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-lg font-bold text-foreground" style={{ fontFamily: "'DM Sans', sans-serif" }}>Update Account Password</h3>
                      <p className="text-xs text-muted-foreground" style={{ fontFamily: "'DM Sans', sans-serif" }}>Change your password regularly for security</p>
                    </div>
                  </div>

                  <form onSubmit={e => { e.preventDefault(); setPassLoading(true); setTimeout(() => setPassLoading(false), 1200); }} className="flex flex-col gap-4">
                    <FieldBox label="Current Password" focused={false}>
                      <div className="flex items-center">
                        <span className="pl-4 text-muted-foreground"><Lock className="w-4 h-4" /></span>
                        <input type="password" required placeholder="••••••••" className="flex-1 px-3 py-3.5 bg-transparent text-foreground placeholder-muted-foreground text-sm outline-none" />
                      </div>
                    </FieldBox>
                    ...
                    <FieldBox label="New Password" focused={false}>
                      <div className="flex items-center">
                        <span className="pl-4 text-muted-foreground"><Lock className="w-4 h-4" /></span>
                        <input type="password" required placeholder="Min. 8 characters" className="flex-1 px-3 py-3.5 bg-transparent text-foreground placeholder-muted-foreground text-sm outline-none" />
                      </div>
                    </FieldBox>
                    ...
                    <FieldBox label="Repeat New Password" focused={false}>
                      <div className="flex items-center">
                        <span className="pl-4 text-muted-foreground"><Lock className="w-4 h-4" /></span>
                        <input type="password" required placeholder="Re-enter new password" className="flex-1 px-3 py-3.5 bg-transparent text-foreground placeholder-muted-foreground text-sm outline-none" />
                      </div>
                    </FieldBox>
...
function MetricCard({ title, value, subtitle, color }: { title: string; value: number; subtitle: string; color: string }) {
  return (
    <div className="bg-[#251508] border border-border rounded-2xl p-6 flex flex-col gap-1.5 relative overflow-hidden shadow-md">
      ...
      <p className={`text-3xl font-black ${color} font-mono`}>
        {fmt(value)}
      </p>
  ...
  return (
    <div className="flex flex-col gap-1.5 text-left">
      ...
      <div className="rounded-xl border transition-all duration-200"
        style={{ borderColor: error ? "rgba(212,24,61,0.6)" : focused ? "rgba(232,160,32,0.7)" : "rgba(232,160,32,0.18)", background: "#2e1a0a" }}>
        {children}
      </div>
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <div className="flex gap-2">
          {(["KG", "PIECE", "GM"] as BazarUnit[]).map(u => (
            <button key={u} type="button" onClick={() => setUnit(u)} className="flex-1 py-2 rounded-xl border text-xs font-bold transition-all cursor-pointer font-mono"
