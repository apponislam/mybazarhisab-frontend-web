...
import {
  ScreenShell, PrimaryButton, BackButton, StepDots,
  FieldBox
} from "@/components/ui/Shared";
import { avatarColor, initials } from "@/lib/mockData";

// Premium Web Layout Wrapper for Desktop View
      ...
      <div className="hidden md:flex md:w-[45%] lg:w-[40%] bg-[#251508] border-r border-[rgba(232,160,32,0.15)] flex-col justify-between p-12 relative overflow-hidden flex-shrink-0 select-none">
        ...
        <div className="flex items-center gap-3 relative z-10">
          ...
          <div>
            ...
            <span className="text-[10px] text-muted-foreground uppercase tracking-widest font-mono">Your Market Companion</span>
          </div>
        </div>
        ...
        <div className="relative z-10 my-auto flex flex-col gap-8">
          <div className="space-y-2">
            ...
            <p className="text-muted-foreground text-sm leading-relaxed" style={{ fontFamily: "'DM Sans', sans-serif" }}>
              Ditch the paper ledger. Track daily bazar shopping lists, rent bills, utility expenses, and group splits in one elegant dashboard.
            </p>
          </div>

          <div className="flex flex-col gap-5">
            {[
              { title: "Split Expenses Dynamically", desc: "Instantly see who spent how much and who owes whom without manual calculations.", emoji: "📊" },
              { title: "Bazar list & quantities", desc: "Log rice, fish, vegetables with price, quantity, and categories in real-time.", emoji: "🛒" },
              { title: "Rent & Bills tracker", desc: "Manage subscriptions, internet bill, electricity, and room rents together.", emoji: "⚡" }
            ].map(f => (
        ...
        <div className="relative z-10 text-xs text-muted-foreground/60 flex items-center justify-between font-mono">
          <span>v1.2.0 • Secure Encryption</span>
          <span>© {new Date().getFullYear()} apponislam</span>
        </div>
      </div>

      {/* Right panel - Form Content card on desktop */}
      <div className="flex-1 flex flex-col justify-center items-center overflow-y-auto px-4 md:px-12 py-8 relative bg-[#1a0e07]">
        <div className="absolute inset-0 pointer-events-none opacity-[0.02]"
          style={{ backgroundImage: "radial-gradient(circle, #e8a020 1px, transparent 1px)", backgroundSize: "28px 28px" }} />
        <div className="w-full max-w-[440px] md:h-[80vh] md:min-h-[580px] md:max-h-[820px] md:border md:border-border md:rounded-[32px] md:bg-card md:shadow-2xl md:overflow-hidden md:flex md:flex-col justify-center">
          {children}
        </div>
      </div>
    </div>
...
export function WebAuthForms({ onLogin }: { onLogin: () => void }) {
  ...
  if (screen === "register") return wrap(<RegisterScreen onBack={() => setScreen("login")} onDone={onLogin} />);
  if (screen === "forgot-email") return wrap(<ForgotEmailScreen onBack={() => setScreen("login")} onNext={em => { setForgotEmail(em); setScreen("forgot-otp"); }} />);
  if (screen === "forgot-otp") return wrap(<ForgotOtpScreen email={forgotEmail} onBack={() => setScreen("forgot-email")} onNext={() => setScreen("forgot-newpass")} />);
  if (screen === "forgot-newpass") return wrap(<ForgotNewPassScreen onBack={() => setScreen("forgot-otp")} onDone={() => setScreen("forgot-success")} />);
  if (screen === "forgot-success") {
    return wrap(
      <ScreenShell scrollable>
        <div className="flex-1 flex flex-col items-center justify-center px-8 py-12 text-center bg-transparent">
  ...
  return wrap(
    <ScreenShell scrollable>
      <div className="flex flex-col flex-1 pb-10 justify-center">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col items-center pt-8 pb-6 px-8">
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
          className="mx-5 md:mx-0 rounded-3xl border-0 md:border-0 bg-transparent p-7 flex flex-col">
          ...
          <form onSubmit={e => { e.preventDefault(); setLoginLoading(true); setTimeout(() => { setLoginLoading(false); onLogin(); }, 1500); }} className="flex flex-col gap-4">
            ...
            <div className="flex justify-end">
              <button type="button" onClick={() => setScreen("forgot-email")} className="text-primary text-sm hover:text-accent transition-colors cursor-pointer" style={{ fontFamily: "'DM Sans', sans-serif" }}>Forgot password?</button>
            </div>
          ...
          <button type="button" onClick={() => setScreen("register")}
            className="w-full py-3.5 rounded-xl border border-border text-foreground text-sm font-medium hover:border-primary/50 hover:bg-primary/5 transition-all cursor-pointer"
    ...
    <ScreenShell scrollable>
      <div className="flex flex-col px-6 pt-12 pb-8">
        ...
        <p className="text-center text-muted-foreground text-sm mt-5" style={{ fontFamily: "'DM Sans', sans-serif" }}>
          ...
          <button type="button" onClick={onBack} className="text-primary hover:text-accent transition-colors font-medium cursor-pointer">Sign In</button>
        </p>
      </div>
    </ScreenShell>
  );
  ...
  return (
    <ScreenShell scrollable>
      <div className="flex flex-col px-6 pt-12 pb-8 flex-1">
        ...
        <form onSubmit={e => { e.preventDefault(); setLoading(true); setTimeout(() => { setLoading(false); onNext(email); }, 1500); }} className="flex flex-col gap-5 flex-1 justify-between">
          ...
          <div className="mt-8 pt-4"><PrimaryButton loading={loading} label="Send OTP" loadingLabel="Sending…" /></div>
        </form>
      </div>
    </ScreenShell>
  );
  ...
  return (
    <ScreenShell scrollable>
      <div className="flex flex-col px-6 pt-12 pb-8 flex-1">
        ...
        <form onSubmit={e => { e.preventDefault(); if (!filled) return; setLoading(true); setTimeout(() => { setLoading(false); onNext(); }, 1500); }} className="flex flex-col gap-6 flex-1 justify-between">
          ...
          <div className="text-center">
            {timer > 0
              ...
              : <button type="button" onClick={() => setTimer(30)} className="text-primary text-sm font-medium hover:text-accent cursor-pointer" style={{ fontFamily: "'DM Sans', sans-serif" }}>Resend OTP</button>}
          </div>
          <div className="mt-8 pt-2"><PrimaryButton loading={loading} label="Verify Code" loadingLabel="Verifying…" disabled={!filled} /></div>
        </form>
      </div>
    </ScreenShell>
  );
  ...
  return (
    <ScreenShell scrollable>
      <div className="flex flex-col px-6 pt-12 pb-8 flex-1">
        ...
        <form onSubmit={e => { e.preventDefault(); if (mm || np.length < 8) return; setLoading(true); setTimeout(() => { setLoading(false); onDone(); }, 1500); }} className="flex flex-col gap-4 flex-1 justify-between">
          ...
          <div className="mt-8 pt-2"><PrimaryButton loading={loading} label="Reset Password" loadingLabel="Saving…" disabled={mm || np.length < 8} /></div>
        </form>
      </div>
    </ScreenShell>
  );
