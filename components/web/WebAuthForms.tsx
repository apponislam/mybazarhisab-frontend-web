...
import {
  ...
  User, Camera, Phone, ArrowLeft
} from "lucide-react";
import { PrimaryButton, SpinnerIcon } from "@/components/ui/Shared";
    ...
    <div className="flex flex-col gap-1.5 text-left">
      ...
      <div 
        className={`flex items-center rounded-xl border transition-all duration-200 ${
          error 
            ? "border-destructive/60 bg-[#2e1a0a] shadow-[0_0_0_3px_rgba(212,24,61,0.08)]" 
            : focused 
              ? "border-primary bg-[#2e1a0a] shadow-[0_0_0_3px_rgba(232,160,32,0.12)]" 
              : "border-[rgba(232,160,32,0.18)] bg-[#2e1a0a]"
        }`}
      >
        <span className="pl-4 text-muted-foreground flex-shrink-0">{icon}</span>
        <input 
          {...props}
          onFocus={(e) => { setFocused(true); props.onFocus?.(e); }}
          onBlur={(e) => { setFocused(false); props.onBlur?.(e); }}
          className="flex-1 px-3 py-3.5 bg-transparent text-foreground placeholder-muted-foreground text-sm outline-none font-sans" 
        />
        {rightElement && <div className="pr-4 flex items-center">{rightElement}</div>}
      </div>
      {error && <p className="text-xs text-destructive mt-0.5" style={{ fontFamily: "'DM Sans', sans-serif" }}>{error}</p>}
    </div>
  );
}
...
export function WebAuthForms({ onLogin, onBack }: { onLogin: () => void; onBack?: () => void }) {
  ...
  if (screen === "forgot-email") {
    return wrap(<WebForgotEmailScreen onBack={() => setScreen("login")} onNext={em => { setForgotEmail(em); setScreen("forgot-otp"); }} />);
  }
...
function WebLoginScreen({
  onLogin, onBack, onRegister, onForgot
}: {
  onLogin: () => void; onBack?: () => void; onRegister: () => void; onForgot: () => void;
}) {
  ...
  return (
    ...
    >
      ...
      <button 
        type="button" 
        onClick={onRegister}
        className="w-full py-3.5 rounded-xl border border-border text-foreground text-sm font-semibold hover:border-primary/50 hover:bg-primary/5 transition-all cursor-pointer"
        style={{ fontFamily: "'DM Sans', sans-serif" }}
      >
        Create an Account
      </button>
  ...
  return (
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        {password.length > 0 && (
          <div className="flex items-center gap-3 -mt-2 select-none">
            ...
            <span className="text-xs font-semibold font-mono" style={{ color: sc[sl] }}>{slb[sl]}</span>
          </div>
        )}
      ...
      <p className="text-center text-muted-foreground text-sm select-none" style={{ fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <button type="button" onClick={onBack} className="text-primary hover:underline font-semibold cursor-pointer">Sign In</button>
      </p>
    </motion.div>
  );
}
  ...
  return (
    ...
    >
      ...
      <div className="w-full">
        <button 
          onClick={onDone}
          className="w-full py-4 rounded-xl bg-primary text-primary-foreground font-semibold text-base hover:bg-accent transition-all cursor-pointer shadow-lg shadow-primary/20"
        >
          Back to Sign In
        </button>
      </div>
