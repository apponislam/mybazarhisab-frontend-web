  ...
  return (
    <ScreenShell scrollable>
      <div className="w-full max-w-5xl mx-auto flex flex-col min-h-full justify-center py-12 px-6 md:px-12 bg-transparent">
        ...
        <div className="h-px bg-border mb-10 flex-shrink-0" />

        {/* Dual Panel Layout: Join Group & Create Group side-by-side */}
        <div className="flex-1 flex flex-col md:flex-row md:items-stretch md:justify-center gap-8">
          
          {/* Card 1: Join a Group */}
          <motion.div 
            initial={{ opacity: 0, y: 20 }} 
            animate={{ opacity: 1, y: 0 }} 
            className="flex-1 rounded-3xl border border-border bg-card p-8 flex flex-col justify-between shadow-xl" 
                  ...
                  >
                    {cl ? (
                      ...
                      <span className="flex items-center justify-center gap-2">
                        <Plus className="w-4 h-4 text-accent" strokeWidth={2} />Create Group
                      </span>
                    )}
                  </motion.button>
                </div>
