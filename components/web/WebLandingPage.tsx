...
import { ShoppingBag, Receipt, Users, ArrowRight, CheckCircle2, TrendingUp, BarChart3, ShieldCheck } from "lucide-react";
...
export function WebLandingPage({ onSignIn }: { onSignIn: () => void }) {
    return (
        <div className="min-h-screen bg-[#1a0e07] text-[#f5ede2] flex flex-col font-sans overflow-x-hidden selection:bg-primary selection:text-primary-foreground relative">
            ...
            <div className="absolute top-0 left-0 w-full h-[600px] pointer-events-none" style={{ background: "radial-gradient(circle at 50% -100px, rgba(232,160,32,0.18) 0%, transparent 80%)" }} />
            ...
            <header className="w-full relative z-10 border-b border-[rgba(232,160,32,0.1)] select-none">
                <div className="max-w-6xl mx-auto px-6 h-20 flex items-center justify-between">
                    ...
                    <nav className="hidden md:flex items-center gap-8">
                        ...
                        <a href="#steps" className="text-sm text-muted-foreground hover:text-primary transition-colors">
                            How it Works
                        </a>
                    ...
                    <button onClick={onSignIn} className="px-5 py-2.5 bg-primary/10 border border-primary/30 text-primary rounded-xl text-sm font-semibold hover:bg-primary hover:text-primary-foreground transition-all cursor-pointer shadow-inner">
                        Sign In
                    </button>
                </div>
            </header>
            ...
            <section className="relative z-10 max-w-6xl mx-auto px-6 pt-16 md:pt-24 pb-16 flex-1 flex flex-col md:flex-row items-center gap-12">
                <div className="flex-1 text-center md:text-left space-y-6">
                    <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-primary/20 bg-primary/5 text-primary text-xs font-semibold font-mono">
                        <span>✨</span> THE DEDICATED ROOM ACCOUNT COMPANION
                    </motion.div>

                    <motion.h2 initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="text-4xl md:text-5xl lg:text-6xl font-extrabold tracking-tight text-foreground leading-tight" style={{ fontFamily: "'DM Sans', sans-serif" }}>
                    ...
                    <motion.p initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="text-muted-foreground text-sm md:text-base leading-relaxed max-w-lg">
                    ...
                    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.3 }} className="flex flex-col sm:flex-row items-center gap-4 justify-center md:justify-start pt-2">
                        <button onClick={onSignIn} className="w-full sm:w-auto px-8 py-4 bg-primary text-primary-foreground font-semibold text-base rounded-xl hover:bg-accent transition-all cursor-pointer shadow-lg shadow-primary/20 flex items-center justify-center gap-2">
                            Get Started Now <ArrowRight className="w-4 h-4" />
                        </button>
                        <a href="#features" className="w-full sm:w-auto px-8 py-4 border border-border bg-[#2e1a0a]/50 text-foreground font-semibold text-base rounded-xl hover:bg-[#2e1a0a] transition-all flex items-center justify-center cursor-pointer">
                            Learn More
                        </a>
                    </motion.div>
                </div>
                ...
                <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.25, duration: 0.6 }} className="flex-1 w-full max-w-[500px]">
                    <div className="rounded-[32px] border border-border bg-card p-6 shadow-2xl relative overflow-hidden" style={{ boxShadow: "0 20px 80px rgba(0,0,0,0.6)", backdropFilter: "blur(8px)" }}>
            ...
            <section id="features" className="relative z-10 bg-[#251508]/40 border-y border-[rgba(232,160,32,0.1)] py-20 select-none">
                <div className="max-w-6xl mx-auto px-6">
                    <div className="text-center max-w-2xl mx-auto space-y-3 mb-16">
                        ...
                        <h2 className="text-3xl md:text-4xl font-extrabold" style={{ fontFamily: "'DM Sans', sans-serif" }}>
                            Designed for Modern Shared Rooms
                        </h2>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        {[
                            ...
                            <div key={i} className="p-8 bg-[#251508] border border-border rounded-3xl hover:border-primary/40 hover:-translate-y-1 transition-all duration-300 shadow-lg">
                                <div className="w-12 h-12 rounded-2xl bg-[#1a0e07] border border-border flex items-center justify-center mb-6 shadow-inner">{f.icon}</div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-12 relative">
                        ...
                        {[
                            ...
                            <div key={i} className="flex flex-col items-center text-center space-y-4 relative z-10">
                                ...
                                <h4 className="text-lg font-bold text-foreground" style={{ fontFamily: "'DM Sans', sans-serif" }}>
                                    {step.title}
                                </h4>
                ...
                <div className="max-w-6xl mx-auto px-6">
                    <div className="flex flex-col md:flex-row items-center gap-12">
                        <div className="flex-1 space-y-6">
                            ...
                            <p className="text-muted-foreground text-sm leading-relaxed">Money matters in a shared flat can get complex. My Bazar Hisab ensures full transparency and accuracy, keeping accounts stress-free.</p>

                            <div className="space-y-4">
                                {["100% accurate split billing algorithms.", "Saves time: no manual record-keeping or calculator sessions.", "Clear, auditable logs: everyone see who bought what.", "Safe secure data, accessible from both desktop & mobile."].map((text, i) => (
            ...
            <footer className="relative z-10 border-t border-[rgba(232,160,32,0.1)] py-12 select-none bg-[#100905]">
                <div className="max-w-6xl mx-auto px-6 flex flex-col sm:flex-row items-center justify-between gap-6">
                    ...
                    <p className="text-xs text-muted-foreground/60 font-mono text-center sm:text-left">© {new Date().getFullYear()} My Bazar Hisab. Built for room mates and flat mates.</p>

                    <button onClick={onSignIn} className="px-6 py-3 bg-primary text-primary-foreground font-semibold text-xs rounded-xl hover:bg-accent transition-all cursor-pointer shadow-md shadow-primary/10">
                        Launch Web App
                    </button>
                </div>
            </footer>
