...
export function WebDialogModal({
    show,
    onClose,
    title,
    children,
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center">
                    ...
                    >
                        <div className="flex items-center justify-between mb-6">
                            ...
                            <button
                                onClick={onClose}
                                className="w-8 h-8 rounded-full bg-[#2e1a0a] flex items-center justify-center text-muted-foreground hover:text-foreground transition-colors cursor-pointer"
                            >
                                <X className="w-4 h-4" />
                            </button>
                        </div>
...
export function WebAddExpenseForm({
    onSubmit,
    onClose,
    isLoading,
}: {
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 text-left">
            ...
            <div className="grid grid-cols-2 gap-4">
                <div className="flex flex-col gap-1.5">
                    ...
                    <input
                        ...
                        required
                        placeholder="0.00"
                        className="w-full px-4 py-3 bg-[#2e1a0a] border border-border rounded-xl text-sm outline-none font-mono text-foreground"
                    />
                </div>
                <div className="flex flex-col gap-1.5">
            ...
            <div className="flex flex-col gap-1.5">
                ...
                <input
                    ...
                    required
                    className="w-full px-4 py-3 bg-[#2e1a0a] border border-border rounded-xl text-sm outline-none text-foreground font-mono"
            ...
            <div className="flex gap-3 mt-4">
                <button
                    type="button"
                    onClick={onClose}
                    className="flex-1 py-3 border border-border text-foreground font-bold rounded-xl transition-all hover:bg-secondary cursor-pointer"
                >
                    Cancel
                </button>
                <button
                    type="submit"
                    disabled={isLoading}
                    className="flex-1 py-3 bg-primary text-primary-foreground font-bold rounded-xl transition-all hover:bg-accent cursor-pointer disabled:opacity-50"
                >
...
export function WebAddBillForm({
    onSubmit,
    onClose,
    isLoading,
}: {
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 text-left">
            <div className="grid grid-cols-2 gap-4">
                <div className="flex flex-col gap-1.5">
                    ...
                    <select
                        ...
                        className="w-full px-4 py-3 bg-[#2e1a0a] border border-border rounded-xl text-sm outline-none font-sans text-foreground"
                        style={{ colorScheme: "dark" }}
                    >
            ...
            <div className="flex flex-col gap-1.5">
                ...
                <input
                    ...
                    required
                    placeholder="e.g. July House Rent, Wi-Fi Bill"
                    className="w-full px-4 py-3 bg-[#2e1a0a] border border-border rounded-xl text-sm outline-none text-foreground"
                />
            </div>
            <div className="flex flex-col gap-1.5">
            ...
            <div className="flex flex-col gap-4 text-left">
                <div className="p-4 rounded-xl bg-secondary/30 border border-border flex flex-col gap-2">
                    ...
                    <div className="flex items-center gap-1 my-1">
                        {[1, 2, 3, 4, 5].map((star) => (
                            <Star
                                key={star}
                                className={`w-5 h-5 ${star <= (userReviewState.review?.rating || 0) ? "text-amber-400 fill-amber-400" : "text-muted-foreground"}`}
                            />
                        ))}
                    </div>
                ...
                <div className="flex justify-end mt-2">
                    <button
                        type="button"
                        onClick={onClose}
                        className="px-6 py-2.5 border border-border text-foreground font-bold rounded-xl hover:bg-secondary cursor-pointer text-sm"
                    >
                        Close
                    </button>
                </div>
        ...
        <form onSubmit={onSubmitReview} className="flex flex-col gap-4 text-left">
            ...
            <div className="flex gap-3 mt-2">
                <button
                    type="button"
                    onClick={onClose}
                    className="flex-1 py-3 border border-border text-foreground font-bold rounded-xl hover:bg-secondary cursor-pointer"
                >
                    Cancel
                </button>
                <button
                    ...
                    className="flex-1 py-3 bg-primary text-primary-foreground font-bold rounded-xl hover:bg-accent cursor-pointer disabled:opacity-50"
                >
